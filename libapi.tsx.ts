...
function getApiBaseUrl(): string {
  ...
  let url = "https://updated-backend-210c.onrender.com";
...
function readableError(value: unknown): string {
  ...
  if (value && typeof value === "object") {
    const item = value as { message?: unknown; detail?: unknown; msg?: unknown };
    const nested = item.message ?? item.detail ?? item.msg;
...
export async function apiFetch<T>(
  ...
  if (!response.ok) {
    ...
    throw new ApiError(readableError(body.detail ?? body.message ?? `Request failed (${response.status})`), response.status);
  }
