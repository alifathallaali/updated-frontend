...
export const pharmaApi = {
  ...
  askCopilot: (workspace_id: number, message: string, context?: { dataset_id?: number; dataset_version_id?: number; data_context?: string; analysis_type?: string; workspace?: string; filters?: Record<string, unknown> }) => apiFetch<{ content: string; reply?: string; status?: string; provider_used?: string; fallback_used?: boolean; source?: Record<string, unknown> }>("/api/copilot/ask", { method: "POST", body: JSON.stringify({ workspace_id, message, ...context }) }),
